...
class Stem(nn.Module):
    def __init__(self, img_dim=3, out_dim=64, rpe=True):
        ...
        self.conv1 = ConvBNAct(
            img_dim, out_dim // 2, kernel_size=3, stride=2, inplace=True
        )
        self.conv2 = ConvBNAct(
            out_dim // 2, out_dim, kernel_size=3, stride=2, inplace=True
        )
        self.rpe = rpe
        if self.rpe:
...
class LWMSA(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = rearrange(
            qkv,
            "b (qkv h d) (hh ws1) (ww ws2) -> qkv (b hh ww) h d (ws1 ws2)",
            b=B,
            h=self.num_heads,
            d=C // self.num_heads,
            qkv=3,
        ...
        tailor_sum = 1 / (
            self.ws * self.ws
            + torch.einsum("bhnc, bhc->bhn", q, torch.sum(k, dim=-1) + self.eps)
        )
        ...
        v = v.expand(
            B * hh * ww, self.num_heads, C // self.num_heads, self.ws * self.ws
        )
        attn = attn + v
        attn = torch.einsum("bhcn, bhn->bhcn", attn, tailor_sum)
        attn = rearrange(
            attn,
            "(b hh ww) h d (ws1 ws2) -> b (h d) (hh ws1) (ww ws2)",
            b=B,
            h=self.num_heads,
            d=C // self.num_heads,
            ws1=self.ws,
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        self.ps = patch_size
        self.proj_ps = nn.Conv2d(
            img_dim, embed_dim, kernel_size=self.ps, stride=self.ps
        )
        self.proj = nn.Sequential(
            ConvBN(embed_dim, embed_dim, kernel_size=3),
            nn.GELU(),
...
class DetailPath(nn.Module):
    def __init__(self, embed_dim=64):
        ...
        self.dp1 = nn.Sequential(
            ConvBNAct(3, dim1, stride=2, inplace=False),
            ConvBNAct(dim1, dim1, stride=1, inplace=False),
        )
        self.dp2 = nn.Sequential(
            ConvBNAct(dim1, dim2, stride=2, inplace=False),
            ConvBNAct(dim2, dim2, stride=1, inplace=False),
        )
        self.dp3 = nn.Sequential(
            ConvBNAct(dim2, embed_dim, stride=1, inplace=False),
