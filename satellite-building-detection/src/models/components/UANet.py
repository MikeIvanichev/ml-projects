...
class Uncertainty_Rank_Algorithm(nn.Module):
    ...
    def forward(self, map):
        ...
        fore_rank_map[fore_uncertainty_map > 0.0] = 5
        ...
        back_rank_map[back_uncertainty_map > 0.0] = 5
...
class Uncertainty_Aware_Fusion_Module(nn.Module):
    ...
    def forward(self, feature_low, feature_high, map):
        ...
        uncertainty_feature_high = torch.cat(
            (
                uncertainty_fore_map_high * feature_high,
                uncertainty_back_map_high * feature_high,
            ),
            dim=1,
        )
        uncertainty_high_up = F.interpolate(
            self.conv_high(uncertainty_feature_high),
            feature_low.size()[2:],
            mode="bilinear",
            align_corners=True,
        )

        low_map = F.interpolate(
            map, feature_low.size()[2:], mode="bilinear", align_corners=True
        )
        uncertainty_fore_map_low, uncertainty_back_map_low = self.rank(low_map)
        uncertainty_feature_low = torch.cat(
            (
                uncertainty_fore_map_low * feature_low,
                uncertainty_back_map_low * feature_low,
            ),
            dim=1,
        )
...
class PSM(nn.Module):
    ...
    def forward(self, feature, map):
        ...
        for i in range(0, C):
            ...
            proj_query = feature_channel.view(m_batchsize, -1, width * height).permute(
                0, 2, 1
            )
...
class MBDC(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.branch0 = nn.Sequential(BasicConv2d(in_channel, out_channel, 1, 1, 0, 1))
...
class UANet_VGG(nn.Module):
    def __init__(self, channel, num_classes):
        ...
        self.neck = FPN(
            in_channels=[channel, channel, channel, channel], out_channels=channel
        )

        self.decoder = SemanticFPNDecoder(
            channel=channel, feature_strides=[4, 8, 16, 32], num_classes=num_classes
        )

        self.cgm = CGM()
        self.psm = PSM()

        self.ufm_layer4 = Uncertainty_Aware_Fusion_Module(
            high_channel=channel,
            low_channel=channel,
            out_channel=channel,
            num_classes=num_classes,
        )
        self.ufm_layer3 = Uncertainty_Aware_Fusion_Module(
            high_channel=channel,
            low_channel=channel,
            out_channel=channel,
            num_classes=num_classes,
        )
        self.ufm_layer2 = Uncertainty_Aware_Fusion_Module(
            high_channel=channel,
            low_channel=channel,
            out_channel=channel,
            num_classes=num_classes,
        )
        self.ufm_layer1 = Uncertainty_Aware_Fusion_Module(
            high_channel=channel,
            low_channel=channel,
            out_channel=channel,
            num_classes=num_classes,
    ...
    def forward(self, x):
        ...
        predict_5_down = F.interpolate(
            predict_5, layer5.size()[2:], mode="bilinear", align_corners=True
        )
        ...
        return (
            F.interpolate(predict_5, size, mode="bilinear", align_corners=True),
            F.interpolate(predict_4, size, mode="bilinear", align_corners=True),
            F.interpolate(predict_3, size, mode="bilinear", align_corners=True),
            F.interpolate(predict_2, size, mode="bilinear", align_corners=True),
            F.interpolate(predict_1, size, mode="bilinear", align_corners=True),
        )
...
class UANet_Res50(nn.Module):
    def __init__(self, channel, num_classes):
        ...
        self.backbone = resnet50(
            backbone_path="./pretrained_model/resnet50-19c8e357.pth"
        )
...
class UANet_pvt(nn.Module):
    def __init__(self, channel, num_classes):
        ...
        self.backbone = pvt_v2_b0()  # [64, 128, 320, 512] and [32, 64, 160, 256] for b0
        path = "./pretrain_model/pvt_v2_b0.pth"
        ...
        self.conv_2 = nn.Sequential(MBDC(64 // 2, channel))
        self.conv_3 = nn.Sequential(MBDC(128 // 2, channel))
        self.conv_4 = nn.Sequential(MBDC(320 // 2, channel))
        self.conv_5 = nn.Sequential(MBDC(512 // 2, channel))
